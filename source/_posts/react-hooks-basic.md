...
또 useMemo는 최적화의 수단으로 사용되어야 합니다. 미래의 리액트에서는 useMemo로 메모이즈된 값을 "잊어버리는" 방법을 선택할 수도 있습니다. 예를 들어서 스크린에서 벗어난 컴포넌트의 메모리를 해제하는 공식 문서에서는 useMemo없이 코드가 동작하도록 만들고 최적화하는 단계에서 useMemo를 사용하는 것을 권장합니다.
